...
VT1 = VT*cos(BETA);
VT2 = VT*sin(BETA);
HE = HEDEG / 57.3; %Convert degrees to radians
T=0;
TMAX=100;
MISS_DISTANCE=10;
S=0;
...
while RTM > 0 && T <= TMAX && VC > 0
    XNT = 30*sin(T/10);
    if RTM < 1000 H = 0.0002;
    else          H = 0.01;
    end
    BETAOLD=BETA;
    RT1OLD = RT1;
...
title("Two-dimensional tactical missile engagement simulation")
legend("Target","Missile")
xlabel("Downrange (m)")
ylabel("Altitude (m)")

nexttile
plot(ArrayT,ArrayXNTG, ArrayT,ArrayXNCG),grid
title("Two-dimensional tactical missile engagement simulation")
xlabel("Time (s)")
